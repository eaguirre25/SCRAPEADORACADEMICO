...
# ── Exportar asignación documento → tópico (para el dashboard) ───────────────
doc_topic_df <- data.frame(
  doc_index        = seq_len(nrow(prep$meta)),
  filename         = prep$meta$filename,
  doi              = prep$meta$doi,
  titulo           = prep$meta$titulo,
  topico_dominante = apply(modelo_stm$theta, 1, which.max),
  stringsAsFactors = FALSE
)
write_csv(doc_topic_df, "output/document_topics.csv")
cat("document_topics.csv guardado.\n")
