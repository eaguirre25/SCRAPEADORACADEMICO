#!/usr/bin/env python3
"""Inyecta un lector único: visor PDF embebido con fallback a texto/resumen y sitio."""
...
(function(){
  ...
  function iframe(src){return '<iframe src="'+e(src)+'" style="width:100%;height:100%;border:0;background:#fff" allow="fullscreen"></iframe>'}
  function nativePdf(src){
    return '<object data="'+e(src)+'#toolbar=1&navpanes=1&view=FitH" type="application/pdf" style="width:100%;height:100%;background:#fff">'
      +'<div class="fallback"><h3>No se pudo incrustar este PDF</h3><p>El servidor de origen no permite mostrarlo dentro del visor. Usá <b>Texto</b> o <b>Sitio del artículo</b>.</p></div></object>';
  }
  function pdfBody(a){
    ...
    if(pdf){
      return '<div style="height:100%;display:flex;flex-direction:column;background:#fff">'
        +'<div style="padding:6px 10px;background:#fff7e6;color:#684b00;border-bottom:1px solid #e6c76b;font:11px Segoe UI,Arial,sans-serif"><b>PDF del registro</b> · visor embebido directo</div>'
  ...
  async function renderUniversal(a,mode){
    ...
    const bar='<div style="height:44px;display:flex;align-items:center;gap:7px;padding:6px 9px;background:#111820;border-bottom:1px solid #30363d;flex-wrap:wrap">'
      +(hasPdf?'<button id="uvPdf" type="button" style="padding:5px 9px"><b>Visor PDF</b></button>':'')
      +'<button id="uvRead" type="button" style="padding:5px 9px">'+(local||hasCorpus?'Texto completo':'Texto / resumen')+'</button>'
      +(site?'<button id="uvSite" type="button" style="padding:5px 9px">Sitio del artículo</button>':'')
      +'<span style="margin-left:auto;color:#8b949e;font-size:11px">'+(drive?'PDF en Drive':pdf?'PDF remoto':local||hasCorpus?'texto interno':hasAbstract?'resumen':'fuente externa')+'</span></div>';
...
print('Lector único con visor PDF incrustado')
