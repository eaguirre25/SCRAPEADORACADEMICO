...
# Escribir credenciales como JSON de usuario autorizado
creds <- list(
...
writeLines(toJSON(creds, auto_unbox = TRUE), creds_file)

# Autenticar googledrive con el archivo de credenciales
...
# ── Cargar corpus previo (para retomar si se interrumpe) ──────────────────────

if (file.exists(CORPUS_CSV)) {
  corpus_prev  <- read_csv(CORPUS_CSV, show_col_types = FALSE)
  ya_procesados <- corpus_prev$filename
  cat(sprintf("Corpus previo: %d archivos ya procesados. Se saltearán.\n\n",
...
cat(sprintf("Listando PDFs en carpeta Drive: %s\n", folder_id))
