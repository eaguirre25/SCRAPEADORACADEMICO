#!/usr/bin/env python3
"""Actualiza docs/asistente_ia.html para detectar automáticamente si la clave es de Gemini (AIzaSy...) o de Groq (gsk_...) y llamar a la IA correspondiente."""
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .status-tag{font-size:0.75rem;background:#1e293b;color:#94a3b8;padding:4px 12px;border-radius:20px;font-weight:700;border:1px solid #334155}
...
<div class="notebook-container">
  <div class="sidebar">
    <div class="sidebar-section">
      <h3>🔑 Conectar IA Real (Gemini o Groq)</h3>
      
      <div class="form-group">
        <label>Pegar Clave de API (Gemini o Groq):</label>
        <input type="password" id="apiKeyInput" placeholder="Pegá tu clave AIzaSy... o gsk_... aquí">
        <button type="button" id="saveKeyBtn" style="padding:8px;background:#059669;border:none;color:#fff;border-radius:6px;cursor:pointer;font-weight:700;font-size:0.82rem;margin-top:4px">💾 Conectar y Guardar Clave</button>
      </div>

      <div class="key-helper" id="keyHelperBox">
        <b>¿Cómo obtener tu clave gratuita?</b>
        <ol>
          <li><b>Google Gemini (Oficial):</b> Entrá a <a href="https://aistudio.google.com/app/apikey" target="_blank">Google AI Studio</a> y tocá <i>Create API key</i> (copiá la que empieza con <code>AIzaSy...</code>).</li>
          <li><b>Groq Llama 3.3:</b> Entrá a <a href="https://console.groq.com/keys" target="_blank">Groq Keys</a> y tocá <i>Create API Key</i> (empieza con <code>gsk_...</code>).</li>
        </ol>
        <i>Son 100% gratuitas y no requieren tarjeta de crédito.</i>
      </div>
    </div>
  ...
  <div class="chat-area">
    <div class="chat-header">
      ...
      <span class="status-tag" id="statusTag">● Esperando Clave de IA</span>
    </div>
...
function updateStatusUI(key) {
  ...
  if (!key) {
    tag.textContent = '● Modo Respaldo (Sin Key)';
    tag.style.background = '#1e293b';
    tag.style.color = '#94a3b8';
    return;
  }
  if (key.startsWith('AIzaSy')) {
    tag.textContent = '🟢 Google Gemini 1.5 Conectado (2.124 Obras)';
    tag.style.background = '#065f46';
    tag.style.color = '#6ee7b7';
  } else if (key.startsWith('gsk_')) {
    tag.textContent = '🟢 Groq Llama 3.3 Conectado (2.124 Obras)';
    tag.style.background = '#1d4ed8';
    tag.style.color = '#bfdbfe';
  } else {
    tag.textContent = '🟢 IA Conectada';
    tag.style.background = '#065f46';
    tag.style.color = '#6ee7b7';
  }
}
...
document.getElementById('saveKeyBtn').onclick = () => {
  ...
  updateStatusUI(k);
  if(k) {
    alert('✓ Clave guardada exitosamente. Ahora la IA real redactará tus respuestas en vivo.');
  } else {
...
async function sendMessage() {
  ...
  aiDiv.innerHTML = `<div class="notebook-response"><h3>🤖 Consultando el corpus de 2.124 obras...</h3><p>Buscando pasajes literales y generando respuesta...</p></div>`;
  ...
  // 1. Detección automática: Si empieza con AIzaSy... es Google Gemini
  if (apiKey && apiKey.startsWith('AIzaSy')) {
    try {
      container.innerHTML = '<h3>✨ Generando respuesta en vivo con Google Gemini 1.5 Flash...</h3><p>Leyendo y sintetizando pasajes de tus 2.124 obras...</p>';
  ...
  // 2. Detección automática: Si empieza con gsk_... es Groq Llama 3.3
  ...
  if (llmText) {
    ...
    finalHTML += `
      <div style="background:#1e1b4b;border:1px solid #4338ca;padding:14px 18px;border-radius:8px;margin-bottom:16px;font-size:0.9rem">
        💡 <b>Para activar Google Gemini o Llama 3.3 en vivo sobre tus 2.124 obras:</b><br>
        Pegá tu clave gratuita de <a href="https://aistudio.google.com/app/apikey" target="_blank" style="color:#38bdf8;text-decoration:underline">Google AI Studio (empieza con AIzaSy...)</a> o <a href="https://console.groq.com/keys" target="_blank" style="color:#38bdf8;text-decoration:underline">Groq (empieza con gsk_...)</a> en el panel izquierdo y hacé clic en <b>Guardar Clave</b>.
      </div>
    `;
...
print('docs/asistente_ia.html actualizado con autodetección de claves Gemini y Groq.')
